...
### 快速开始
1. 在 Releases 下载 `12-SkillLocalizer-1.1.9-universal.dmg`（见下方「macOS 版本说明」）。
...
hdiutil create -format UDZO -volname SkillLocalizer \
  -srcfolder SkillLocalizer.app 12-SkillLocalizer-1.1.9-universal.dmg
```
（也可直接 `bash build.sh` 一步完成上面两步。）

### macOS 版本说明
- **Apple Silicon + Intel 通用（推荐）**：下载 `12-SkillLocalizer-1.1.9-universal.dmg`，同时包含 arm64 + x86_64，macOS 12.0+ 通吃。

> 该 DMG 为 ad-hoc 签名、**未公证（notarized）**，首次打开请右键「打开」放行 Gatekeeper；在 Apple Silicon 上 Intel 版需通过 Rosetta 2 运行。源码经 `-target` 注入构建，零改动。

> 仓库「发行版 / Releases」的命名格式为：`支持最低版本-SkillLocalizer-版本-架构`（如 `12-SkillLocalizer-1.1.9-universal.dmg`）。
...
### Quick start
1. Download `12-SkillLocalizer-1.1.9-universal.dmg` from Releases (see "macOS build notes" below).
...
### macOS build notes
- **Apple Silicon + Intel (recommended)**: use `12-SkillLocalizer-1.1.9-universal.dmg` (contains both arm64 + x86_64, universal, macOS 12.0+).
...
> Release asset naming: `min-version-SkillLocalizer-version-arch` (e.g. `12-SkillLocalizer-1.1.9-universal.dmg`).
